...
[![png](images/dark-shades.png)]


In this project, Used Python and OpenCV to detect lane lines on the road images.  
...
Finally, Applied all the techniques to process video clips to find lane lines in them.
...
Let's build a filter to select those white and yellow lines.  Want to select particular range of each channels (Hue, Saturation and Light).
...
Used `cv2.GaussianBlur` to smooth out edges.
...
The GaussianBlur takes a `kernel_size` parameter which you'll need to play with to find one that works best.  Tried 3, 5, 9, 11, 15, 17 (they must be positive and odd) and check the edge detection (see the next section) result.  The bigger the `kernel_size` value is,  the more blurry the image becomes.  
...
The project was successful in that the video images clearly show the lane lines are detected properly .
...
Having said that, the lanes near the car are mostly straight in the images.  The curvature appears at further distance unless it's a steep curve.  So, this basic lane detecting technique is still very useful.
